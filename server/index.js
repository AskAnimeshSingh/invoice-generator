...
function formatCurrency(amount) {
  // PDF standard fonts lack ₹ glyph; "Rs." is reliable on Helvetica
...
app.post('/api/generate-invoice', (req, res) => {
  try {
    ...
    doc.on('end', () => {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="invoice-${invoiceNumber || 'draft'}.pdf"`
      );
      res.send(pdfBuffer);
    });
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(28)
      .text('INVOICE', metaX, 28, { width: contentWidth * 0.45, align: 'right' });
    ...
    doc
      ...
      .text('AMOUNT', colAmount, tableTop + 7, { width: 70, align: 'right' });
